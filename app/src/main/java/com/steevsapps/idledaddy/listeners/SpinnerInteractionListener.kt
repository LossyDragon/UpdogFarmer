...
class SpinnerInteractionListener(
    ...
    private fun handleSelection(position: Int) {
        ...
        if (fragment is GamesFragment) {
            when (position) {
                // Library
                GamesFragment.TAB_GAMES -> fragment.switchToGames()

                // Last idling session
                GamesFragment.TAB_LAST -> fragment.switchToLastSession()

                // Blacklisted games
                GamesFragment.TAB_BLACKLIST -> fragment.switchToBlacklist()
            }
        }
    }
}
